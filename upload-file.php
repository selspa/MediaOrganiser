...
if((isset($_POST['file_name']) || $_POST['file_name'] != "") && !in_array($_POST['file_name'], $titles)){
   ...
   if(isset($_FILES['fileToUpload']) && $_FILES['fileToUpload']['error'] == 0){
       ...
       // set files variables
       $target_dir = "uploads/";
       $fileTypes = ["audio/aac", "audio/mp3", "audio/mpeg", "audio/wav", "audio/ogg", "video/mp4", "video/avi"];
       $err_message = "Sorry, you can only upload audio (ACC, MP3, OGG, WAV) or video (MP4, AVI) files ";
     ...
     $f->getImageID($imageName, $image_path);
         ...
         if($_FILES['fileToUpload']['error'] == 1){
           ...
           echo "Error - You need to upload a media file (audio or video)";
         }
       }


 $f->insertFileSpecsInDB($file, $file_name, $file_path, $image_path, $comment, $category, $imageName);

} else {
  if(in_array($_POST['file_name'], $titles)){
